...
function navTabAjaxDone(json){
	...
	if (json.statusCode == DWZ.statusCode.ok){
		...
		if ("closeCurrent" == json.callbackType) {
		...
		}else if("cache"==json.callbackType){
		...
		else if ("auditredirect" == json.callbackType) {
			navTab.reload();
		}
		...
		}else if ("login" == json.callbackType) {//如果callbackType是redirect,就重定向
		  ...
		  //获取带"/"的项目名，如：/uimcardprj
		    ...
		    var localhostPaht=curWwwPath.substring(0,pos)+projectName+json.forwardUrl;
			setTimeout(function(){window.location.replace(localhostPaht)}, 800);
		} else {
